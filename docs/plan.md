...
**Repo today:** only the `.docx` business plan exists — this is a 0→1 build.
...
```
   ...
   T-10 SEED    T-2 REFRESH   T-3h LOCK     ◄─ lifecycle passes
       │            │             │
       ▼            ▼             ▼
API-Football ──► ingest.js ──► SQLite (fixtures, teams, odds, kickoff_utc)
                                       │
                                       ▼
                              generate.js ──► Azure Model Router ──► {Opus | 4o-mini}
                                       │            (returns JSON article)
                                       ▼
                              SQLite (articles + generation_log)
                                       │
                              publish.js ──► affiliate-injector ──► WordPress REST API
                                       │       (upsert: create OR update post_id)
                                       ▼
                              sitemap + GSC submit
                                       │
                    ▼
...
/src
  ...
  /publish   affiliateInjector.js, wordpress.js, sitemap.js
  /config    index.js (env loader)
/scripts     run-ingest.js, run-generate.js, run-publish.js
...
|------|--------------------------------|--------|-------------------|
| **1 — Seed** | T-10 days | Generate + publish with available data (H2H, season form, early odds). Claims the URL so it starts ranking. WordPress returns `wp_post_id`. | `seeded` |
| **2 — Refresh** | T-2 days | Re-ingest fresh data, regenerate, **update the same post** (`wp_post_id`). Peak quiniela search intent. | `refreshed` |
| **3 — Lock** | T-3 hours | Final confirmed-lineup update, in place. | `locked` |

**Scheduler (`scripts/run-cadence.js`):** on each GitHub Actions tick it (1) pulls `wc26.sqlite` from Azure Blob, (2) selects fixtures whose `kickoff_utc` crosses a T-minus threshold AND whose `lifecycle_state` hasn't yet reached that pass, (3) runs ingest → generate → publish for just those, (4) advances `lifecycle_state` + `last_pass` + `last_refreshed_at`, (5) uploads the mutated DB back. Idempotent: re-running a tick is a no-op for already-processed passes.
...
- Secrets (Azure AI endpoint/key, RapidAPI, WordPress app password, Azure Storage connection string) injected as GitHub Actions secrets, mapped to env vars (never committed).
- Steps: checkout → setup Node → `npm ci` → `node scripts/run-cadence.js` → (DB upload handled inside the script).
...
- Caveat: verify the WordPress default `robots.txt` doesn't accidentally block ChatGPT Search (common with security plugins).
...
- Use a WordPress plugin for auto-generation: **All in One SEO** has an LLMs.txt tab, or the **Website LLMs.txt** plugin works standalone.
...
**Structured data (JSON-LD) — inject via `wordpress.js`:**
...
**Implementation**: all disclaimers, responsible-gambling links, affiliate disclosures, and `rel="sponsored"` attributes are **templated into every published page** by `wordpress.js` as constants injected at publish time — never dependent on the LLM generating compliant copy.
...
- Set up Azure AI Foundry project; deploy Claude Opus + GPT-4o-mini; configure model-router in **Cost** mode; capture endpoint + key into `.env`.
- Smoke test: a `scripts/ping-router.js` that sends a trivial prompt to confirm routing works.
...
- Define SQLite schema: `fixtures`, `teams`, `team_stats`, `head_to_head`, `odds`, `articles`, `generation_log`.
  ...
  - `articles` also carries **lifecycle fields** (drive the cadence): `lifecycle_state` (`seeded` → `refreshed` → `locked`), `last_refreshed_at`, `wp_post_id` (returned by WordPress on first publish; reused for in-place updates), and `last_pass` (`seed` | `refresh` | `lock`).
  - **`generation_log`** (cost instrumentation, one row per model call) — the source of truth for cost-per-article:
    ...
    - `model_used` (e.g. `claude-opus` | `gpt-4o-mini` — captured from the router response, so we can see what the **Cost**-mode router actually selected)
...
- **Endpoint mapping (from biz plan §6):** `/fixtures` → `fixtures`; `/teams/statistics` → `team_stats` (squad + recent form). `head_to_head` and `odds` come from API-Football's `/fixtures/headtohead` and `/odds` endpoints (justified by biz plan §4 "historical matchups, live odds").
- Implement `rateLimiter.js` (1 req/sec, honoring RapidAPI quotas).
...
### Phase 3 — Generation Engine (Azure Model Router → JSON articles)
...
- `router.js`: HTTPS call to Azure model-router endpoint; retries + exponential backoff.
...
- **Cost capture (every model call):** `router.js` returns the raw usage block + the router-selected `model` name; `batch.js` writes one `generation_log` row per call (success *and* failure), computing `cost_usd` from `config/pricing.js`. This is non-optional plumbing — without it there is no cost-per-article measurement.
- `scripts/run-generate.js`: concurrency-limited batch runner (e.g. `p-limit(4)`); after each run prints a cost summary (total spend, cost-per-article, model split) from `generation_log`.

### Phase 4 — CMS Integration & Affiliate Monetization
...
- `wordpress.js`: **upsert publisher** — if the article has no `wp_post_id`, POST to `/wp-json/wp/v2/posts` (create) and store the returned ID; if it already has one, POST to `/wp-json/wp/v2/posts/{wp_post_id}` (update in place). Auth via application password; map JSON → title/slug/content/meta; status=`publish`. This is what makes the Seed→Refresh→Lock passes update the *same* page instead of creating duplicates.
- `scripts/run-publish.js`: pulls articles needing publish/refresh for the current pass, injects affiliates, upserts to WordPress, persists `wp_post_id`, and advances `lifecycle_state`/`last_pass`/`last_refreshed_at`.

### Phase 5 — Execution & Indexing
- `sitemap.js`: regenerate XML sitemap from published posts.
- Submit to Google Search Console (manual or via Search Console API stretch goal).
- Add a top-level `npm run pipeline` (ingest → generate → publish → sitemap).
- Azure log review checklist to confirm router cost-splitting between Opus and 4o-mini.
- **`scripts/cost-report.js`** (the cost-per-article truth source) — queries `generation_log` and prints:
  ...
  - **Model split**: % of calls (and % of spend) routed to Opus vs 4o-mini — validates the **Cost**-mode router is actually offloading light work to the mini model.
...
- [ ] Deployed models: **Claude Opus** + **GPT-4o-mini** + **model-router** (Cost mode)
- [ ] Azure endpoint URL + API key → `.env`
- [ ] **RapidAPI** account + **API-Football** subscription (free tier or paid for WC volume) → `.env`
- [ ] **WordPress** instance (self-hosted or managed) with REST API enabled + application password
- [ ] **Azure Blob Storage** account + container for the SQLite DB; capture the **connection string** (or SAS token) for the GitHub Action
- [ ] **GitHub repository** with Actions enabled; add secrets: `AZURE_AI_ENDPOINT`, `AZURE_AI_KEY`, `RAPIDAPI_KEY`, `WP_APP_PASSWORD`, `AZURE_STORAGE_CONNECTION_STRING`
...
- [ ] **Verify Azure model-router response** exposes model identity + token usage (make one real test call, inspect raw JSON)
- [ ] **WordPress LLMs.txt + schema plugin** (All in One SEO or Website LLMs.txt plugin) installed and configured
- [ ] **Failure alerting webhook** (Discord/Telegram) wired to GitHub Actions `if: failure()` step
- [ ] **Author profiles** with bios + credentials created in WordPress (E-E-A-T requirement; link from article bylines)
...
- Azure model-router cost overruns if Opus is over-selected — instrument token logging from day one.
...
- **Azure model-router opaque metadata (High/High)**: cost tracking assumes the router response reveals which model (Opus vs 4o-mini) and token usage. Foundry may return only a deployment alias. MITIGATE: make one real call now and inspect raw JSON; if model identity is hidden, fall back to two explicit deployments routed in our own code; set an Azure spend cap regardless.
...
- **Opus cost during development (Med-High/High)**: uncontrolled prompt iteration × Opus pricing burns budget pre-revenue. MITIGATE: $100/month hard cap; use GPT-4o-mini for ALL dev/testing, Opus only for production runs; cache by input-data hash to skip unchanged fixtures.
...
- **WordPress API throttling/perf (Med)**: bulk publish can trip host WAF; cheap shared hosting slows under 200+ posts. MITIGATE: throttle to 2 posts/min; managed WP host with page caching; dedicated Author-role bot user; rotate the Application Password monthly.
